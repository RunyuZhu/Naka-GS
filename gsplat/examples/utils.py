...
class AppearanceOptModule(torch.nn.Module):
    ...
    def forward(
        ...
        if embed_ids is None:
            camera_embeds = torch.zeros(C, self.embed_dim, device=features.device)
        else:
            camera_embeds = self.embeds(embed_ids)  # [C, D2]
        return self.forward_with_camera_embeds(
            features=features,
    ...
    def forward_with_camera_embeds(
        self, features: Tensor, camera_embeds: Tensor, dirs: Tensor, sh_degree: int
    ) -> Tensor:
        from gsplat.cuda._torch_impl import _eval_sh_bases_fast

        C, N = dirs.shape[:2]
        embeds = camera_embeds[:, None, :].expand(-1, N, -1)  # [C, N, D2]
        # GS features
